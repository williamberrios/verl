...
class CalculateFactualityScoreAggFactuality(orch.ListFunctionComponent):
    def forward(self, grouped_scores, fixed_knowledge, grouped_claims):
        fct_scores = []
        ...
        for scores, knowledge, claims in zip(
            ...
            if orch.is_invalid(claims):
                ...
                status = (
                    "success"
                    if any(not orch.is_invalid(claim_score) for claim_score in scores)
                    else "failure"
                )
                fct_scores.append(
                    {
...
class FactualityPipeline(orch.Pipeline):
    ...
    def forward(self, data):
        ...
        flat_claims = orch.realize(flat_claims)
...
@reward_function("factuality_weighted_reward_batch")
def factuality_weighted_reward_batch(data_sources: List[str],
    ...
    results = factuality_pipeline(data)
